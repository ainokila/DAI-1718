...
# Valores de los parametros con los que podemos ver algo interesante:
# x1: -1.0
# y1: -1.0
# x2: 1.0
# y2: 1.0
...
def getColorPaleta(paleta, nColoresPaleta, color):
    ...
    icolor1 = float(nuevoColor) * float(nPuntosControlPaleta -
                                        1) / float(nColoresPaleta - 1)
   ...
   # print int(icolor1)
    ...
    nr = int(paleta[icolor1][0] * porcentajeC1 +
             paleta[icolor2][0] * porcentajeC2)
    ng = int(paleta[icolor1][1] * porcentajeC1 +
             paleta[icolor2][1] * porcentajeC2)
    nb = int(paleta[icolor1][2] * porcentajeC1 +
             paleta[icolor2][2] * porcentajeC2)
...
def renderizaMandelbrotBonito(x1, y1, x2, y2, ancho, iteraciones, nombreFicheroPNG, paleta, nColoresPaleta):
    ...
    for y in range(imgy):
        ...
        for x in range(imgx):
            ...
            if (i == maxIt - 1):
                # El nucleo del fractal en negro
                image.putpixel((x, y), (0, 0, 0))
            else:
                image.putpixel((x, y), getColorPaleta(
                    paleta, nColoresPaleta, i))
